...
def filtrar_datos_fn(**context):
    service_account_info = {key} ## para que no nos bajen
...
def top_product_fn(**context):
    ...
    fecha_str = datetime.strptime(context["ds"], "%Y-%m-%d")
    hoy = fecha_str + timedelta(days=1)
...
def db_writing_fn(**context):
    ...
    fecha_str = datetime.strptime(context["ds"], "%Y-%m-%d")
    execution_date = fecha_str + timedelta(days=1)
    hoy = execution_date.strftime("%Y-%m-%d")

    top_product_path = f'gs://adtech-tp-2025/top/top_product/top_product_{hoy}.csv'
    top_ctr_path = f'gs://adtech-tp-2025/top/top_ctr/top_ctr_{hoy}.csv'
